...
public class LeaveWordRepository(IUnitOfWorkManage unitOfWorkManage) : BaseRepositories<LeaveWord>(unitOfWorkManage), ILeaveWordRepository {
    ...
    public async Task<List<LeaveWord>> GetBackList(string? userName, int? isCheck, string? startTime, string? endTime) {
        ...
        if (isCheck != null) query = query.Where(lw => lw.IsCheck == isCheck.Value);
    ...
    public async Task<bool> SetIsChecked(long leaveWordId, int isChecked) {
        return await Db.Updateable<LeaveWord>().SetColumns(lw => lw.IsCheck == isChecked).Where(lw => lw.Id == leaveWordId).ExecuteCommandAsync() > 0;
    }
}